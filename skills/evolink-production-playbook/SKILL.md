...
4. 抓总只按 `producer-orchestrator.md` 调度；发话术优先直接复用 `phase-command-templates.md`，保证每阶段命令模板化、可复制。
5. 用户确认工作流后，按 skill 配置的机器人名单逐一点名 @机器人名称 发起 ACK（@编剧/@导演/@视效师/@剪辑师），待全员回复后再进入第一棒。
