...
4. 抓总只按 `producer-orchestrator.md` 调度，不代替岗位产出。
5. 各岗位只读取自己的 SOP 模块，不串岗。
6. 全员必须遵守 `handoff-protocol.md` 回传格式与验收门禁；在 Discord 频道内默认启用抗截断短模板回传。
7. 多 bot 视频任务默认在**任务线程**中启动与推进，主频道只做分发、确认与总结。
8. 视频时长超过单模型能力时，按模板分段；失败仅单段重试。
